 ...
 * Penn email: <jagsingh@seas.upenn.edu>
 * Date: 2022-03-01
 */
...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    protected Node findNode(E val) {
        //Find node acts as a wrapper to contain search method
    ...
    protected int depth(E val) {
        int depth = 0;//Init depth value
        if (val == null) {return -1;}//If the target node is null return null
        return depthValue(root,val,depth);//Run depth value method recursively until depth is found
    }
    ...
    /**
     * Contains search method that is to be run recursivly during findNode method
     */
    private Node search(Node current, E target){
        if (target == null) return null;//Edge case to return null if current node is null (not found)

        if (current.value.compareTo(target) == 0) {return current;} //Return current node if target is found

        //Run search again on left child if current node is greater than target
        else if (current.value.compareTo(target) > 0){
            return search(current.left, target);
        }

        //Run search again on right child if current node is less than target
        else {return search(current.right,target);}
    }

    /**
     * Returns depth of a specfic node
     */
    private int depthValue(Node current, E target, int depth){
        if (current == null) {return -1;}//If the root node is null return -1, or we reach a null node (value not found)

        //If the target equals the current node return the depth as is
        if (this.root.value.compareTo(target)==0){return depth;}

        //If the target is less than the current node increment the depth and re-run method on left child
        else if(this.root.value.compareTo(target) > 0){
            depth++;
            depthValue(current.left, target, depth);
        }
        //If the target is greater than the current node increment the depth and re-run method on right child
        else {
            depth++;
            depthValue(current.right, target, depth);
        }
        return depth;
    }
